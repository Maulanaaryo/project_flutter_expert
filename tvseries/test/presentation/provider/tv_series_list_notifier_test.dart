...
void main() {
  ...
  group('now playing tv', () {
    test('initialState should be Empty', () {
      expect(provider.nowPlayingTvState, equals(RequestState.HasEmpty));
    });
    ...
    test('should return error when data is unsuccessful', () async {
      ...
      await provider.fetchNowPlayingTvSeries();
      // assert
      expect(provider.nowPlayingTvState, RequestState.HasError);
  ...
  group('popular tv', () {
    ...
    test('should return error when data is unsuccessful', () async {
      ...
      await provider.fetchPopularTvSeries();
      // assert
      expect(provider.popularTvState, RequestState.HasError);
      expect(provider.message, 'Server Failure');
      expect(listenerCallCount, 2);
    });
  });
  ...
  group('top rated tv', () {
    ...
    test('should return error when data is unsuccessful', () async {
      ...
      await provider.fetchTopRatedTvSeries();
      // assert
      expect(provider.topRatedTvState, RequestState.HasError);
      expect(provider.message, 'Server Failure');
      expect(listenerCallCount, 2);
    });
  });
